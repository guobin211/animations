...
export class GravityComponent extends AutoCanvasComponent implements AfterViewInit {
  constructor() {
    super();
    this.tsCode = TS_CODE;
  }
...
const animateFn: AnimateFn = (canvas, context, callback) => {
  ...
  function checkCollision(ball0: Ball, ball1: Ball) {
    ...
    if (dist < ball0.radius + ball1.radius) {
      ...
      vel0.x =
        ((ball0.mass - ball1.mass) * vel0.x + 2 * ball1.mass * vel1.x) / (ball0.mass + ball1.mass);
